...
> 가장 큰 도약은 모델이 아니라 **데이터 안의 사실을 찾아낸 데서** 나왔고, 자체 감사로 **규정 위반을 스스로 찾아 점수를 깎아가며 고쳤다.**
...
| 항목 | 내용 |
|---|---|
| 대회 | 제3회 풍력발전량 예측 AI 경진대회 BARAM 2026 (Dacon · KPX), 2026.07 – 2026.08, 1인 |
| 대상 | 태백 가덕산 풍력단지 3개 KPX 그룹 (Vestas / Unison 5기 등) |
| 입력 | day-ahead NWP: **LDAPS 16격자** (50 m·10 m 바람 등) + **GFS 9격자** (850 hPa 등), 학습기간 SCADA(터빈별 출력·풍속) |
| 출력 | 2025년 **8,760시간 × 3그룹** 발전량(kWh) — 매일 D-1 13:00 KST에 발표된 예보로 D일 24시간 예측 |
| 학습 | 2022-01 – 2024-12 라벨 26,304행 (g3는 2023년 준공이라 2년치) |
...
```
FICR은 **KPX 재생에너지 발전량 예측제도의 정산금 산식** 그대로다. 오차 8% 이내에 들어가면 돈을 받고 아니면 0 — **계단형 지시함수 손실**이라 평균 오차를 줄이는 것과 밴드 안에 들어가는 것이 다른 문제가 된다. price(e) = 3·1{e≤8%} + 1·1{e≤6%}로 분해되므로 ±8% 창이 지배하고, 실측 가중이라 고출력 시각이 결정적이다. 프로젝트 후반의 거의 모든 의사결정이 이 구조에서 나왔다.
...
| 최종 파이프라인 (규정 준수 · 비트단위 재현 검증) | 0.65799 (1−NMAE 0.8772 / FICR 0.4387) | — |
...
이건 운이 아니라 설계다. 프로젝트 중반에 **공개 LB에서 최댓값을 고르면 Private를 잃는다**는 것을 정량화했고(분할 간 Public/Private 상관 −0.9999, 페어 비교 표준편차 0.0006–0.0009), 이후 모든 제출은 **사전 등록한 채택 문턱**(+0.0012)으로만 판정했다. 61회 제출 중 "LB 튜닝"성 제출은 조건부 보정 창을 정하는 3–4회의 **분해 프로브**뿐이며, 그것도 그룹·분기 단위 기여를 분리하는 용도였다.
...
| 단계 | 변경 | Public | Δ |
|---|---|---|---|
| 베이스라인 | LightGBM(L1) + CF 타깃 + α/floor 후처리 | 0.6362 | — |
| CF 가중 | + CF 샘플가중 (채점 필터 반영) | 0.6399 | +0.0037 |
| MLP | + PyTorch MLP 앙상블 | 0.6436 | +0.0037 |
| SeqCNN | + SeqCNN (24h 블록 dilated 1D-CNN) → **3-way** | 0.6478 | +0.0042 |
| ECMWF | + **ECMWF IFS** 850/925 hPa (외부데이터, 직접 수집) | 0.6500 | +0.0022 |
| 멤버별 용량 | 확장 ECMWF를 **g2 GBM 멤버에만** | 0.6504 | +0.0004 |
| 터빈 정지 | **g3 터빈 정지 발견** → 계단형 보정 (Q1 ×0.88 / Q2 ×0.95) | 0.6570 | **+0.0067** |
| 계절 보정 | Q1 g1·g2 ×0.96 | 0.6583 | +0.0013 |
| 규정 자체 감사 | ECMWF 런시각·보간 규정 위반 2건 적발·수정 | 0.6556 | **−0.0027 (감수)** |
| EC 멤버 | + ECMWF 전용 4번째 멤버 (입력 정보원 분리) | 0.6568 | +0.0012 |
| 고풍속 게이트 | **g3 고풍속 ×1.06** (라벨 오염의 증상 보정) | **0.6580** | +0.0012 |
</details>
...
LDAPS 16격자 + GFS 9격자 + (ECMWF IFS 직접수집)
        │  103개 피처: 풍속·풍향 sin/cos·공기밀도·ws³·시어·인접시각 램프·공간 산포
        ▼
 ┌─ LightGBM (L1, 3 seed) ─┐
...
- **모델**: LightGBM(objective l1, num_leaves 63, lr 0.04, 1500 trees) / MLP([256,128,64], BN·Dropout 0.2, 가중 L1) / SeqCNN(24h 예보블록, dilated Conv1d 1·2·4) — 각 시드 3개 평균, 3-way 등가중.
...
- **외부데이터**: ECMWF IFS open data를 직접 수집(D-2 18Z 런만, 공개 시각이 예측기준시점보다 앞섬을 실측으로 확인). 별도 멤버로 쓰면 기존 멤버와의 잔차상관이 0.91 → 0.77–0.84로 내려가 유일하게 다양성을 만든 정보원이었다.
...
| 규칙 | 근거 |
|---|---|
| **matched-pair**만 신뢰: 프로덕션 실물 base에 변경 하나만 얹고 후처리 고정, LOYO | 정보 추가 레버 3전 3승 (전이율 74–98%) |
...
학습기간 SCADA를 상태기계(정지=풍속>4 m/s & 출력≈0, 가동=출력>5%)로 읽으니 **g3 5기 중 1기가 2024-12-02부터 학습 종료까지 완전 정지**였고, 학습행의 **33%가 부분 정지 상태**였다.
...
- 정지 오염은 **고풍속 라벨을 4/5로 눌러** 모델에 과소예측을 학습시킨다. 마지막 주에 세 개의 독립 실험(밴드최적 결정규칙·가법 후처리·멤버스프레드)이 전부 "g3 고풍속대"를 가리켜 → **예보 허브풍속 ≥ 학습 상위 30% 시각만 ×1.06** → +0.0012. 라벨 자체를 복원해 재학습하는 근원 치료는 폴드 간 충돌로 기각했다 — **작고, 양 폴드에서 검증되고, 게이트가 좁은 개입만 전이된다.**
...
1. **런시각 선택**: 병렬 다운로드의 완료 순서에 따라 중복 제거가 경계 시각에 **D-1 18Z 런(기준시점보다 14시간 뒤)** 을 택함 → 2025 시각의 17.0%에 영향. 각 행의 런시각을 명시적으로 계산해 기준시점 이후 런을 배제.
2. **잔여 보간**: 1차 수정 후 남은 결측을 시간 선형보간이 채웠는데 **보간의 최대 칸 수 제한은 채우는 칸 수만 제한하고 앵커 거리는 제한하지 않는다** ([0, NaN×26, 100]에서 두 번째 값 = 3.70 = 100/27). 129시각 오염. 앵커의 합법성(기준시점 이전 공개)까지 검사하는 보간으로 교체.

수정으로 Public **0.6583 → 0.6556 (−0.0027)** 을 감수했고, 이후 규정 안에서 다시 0.6580까지 회복했다. 최종 파이프라인은 처음부터 다시 실행해 **비트단위 재현**을 확인했고(3그룹 최대 |Δ| = 0.0 kWh), 프로덕션 경로 전수 점검(LDAPS/GFS 보간 앵커, ECMWF 런선택·보간, 결측 대체 통계, NN 스케일러, 예보 lead 참조)까지 완료했다.
...
## 사용 도구
Python 3.13 · pandas / numpy / scipy · LightGBM · PyTorch (MLP·SeqCNN) · scikit-learn · Herbie / xarray (ECMWF 수집) · matplotlib
